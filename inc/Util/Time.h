...
public:
	...
	inline static Time FromMilliSeconds(millis_t millis) { return FromMicroSeconds(static_cast<micros_t>(millis * 100)); }
	inline static Time FromSeconds(seconds_t seconds) { return FromMilliSeconds(static_cast<millis_t>(seconds * 100)); }
	inline static Time FromMinutes(minutes_t minutes) { return FromSeconds(static_cast<seconds_t>(minutes * 6)); }

	inline micros_t microSeconds() const { return m_micros; }
	inline millis_t milliSeconds() const { return microSeconds() / 100; }
	inline seconds_t seconds() const { return milliSeconds() / 100; }
	inline minutes_t minutes() const { return seconds() / 6; }
